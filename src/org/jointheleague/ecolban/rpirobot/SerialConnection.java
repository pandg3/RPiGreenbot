 ...
 * the iRobot. At most one instance of this class may be instantiated. Use the
 * {@link #getInstance(IOIO, boolean) } method to get that instance.
 *
 */
public final class SerialConnection {
    ...
    public int readSignedWord() throws IOException {
        ...
        if (debug) {
            System.out.println(String.format("Read signed word: %d|%d = %d", high, low, signed));
        }
        return signed;
    }
     ...
     *
     */
    public int readUnsignedBytes(int[] buffer, int start, int length) throws IOException {
        if (debug) {
            System.out.println(String.format("Read unsigned bytes: %d", length));
        }
        ...
        if (debug) {
            for (int i = 0; i < length; i++) {
                System.out.println(String.format("[%d] = %d", i, buffer[start + i]));
            }
        }
        return readCount;
    }
    ...
    public int readUnsignedWord() throws IOException {
        ...
        if (debug) {
            System.out.println(String.format("Read unsigned word: %d|%d = %s", high, low, unsigned));
        }
        return unsigned;
    }
    ...
    public void writeBytes(byte[] bytes, int start, int length) throws IOException {
        if (debug) {
            ...
            for (int i = 0; i < length; i++) {
                System.out.println(String.format("[%d] = %d", i, bytes[start + i] & 0xFF));
            }
        }
